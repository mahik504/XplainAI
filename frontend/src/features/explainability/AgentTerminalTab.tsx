...
export const AgentTerminalTab: React.FC<AgentTerminalTabProps> = ({
  ...
  const defaultLogs = [
    ...
    { time: "00:00:04.0", agent: "SYNTHESIS_CORE", message: "Constructed 3D knowledge topology and observable assertions." },
  ];
  ...
  return (
    <div className={cn("flex flex-col gap-3 font-mono text-xs", className)}>
      {/* Terminal Title Bar */}
      <div className="flex items-center justify-between rounded-t-lg border border-white/[0.08] bg-black/60 px-3 py-2">
        ...
        <Button
          ...
          variant="ghost"
          onClick={handleCopy}
          className="h-6 gap-1 px-2 text-[10px] text-slate-400 hover:bg-white/[0.06] hover:text-white"
        >
      ...
      {/* Terminal Body */}
      <div className="relative max-h-96 overflow-y-auto rounded-b-lg border border-t-0 border-white/[0.08] bg-[#070b16]/95 p-3 shadow-inner scrollbar-slim">
        <div className="space-y-1.5 leading-relaxed">
          ...
          {stageEvents.map((evt, idx) => (
            <div key={`stage-${idx}`} className="flex items-start gap-2">
              <span className="shrink-0 text-[10px] text-slate-500">[STAGE]</span>
              <span className="shrink-0 text-[10px] font-bold text-indigo-400">[{evt.stage.toUpperCase()}]</span>
