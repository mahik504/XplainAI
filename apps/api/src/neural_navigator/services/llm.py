...
class OpenAICompatibleProvider:
    ...
    name: str = LLMProviderName.OPENAI.value
...
class OmniRouterProvider(OpenAICompatibleProvider):
    """Client for OpenRouter / Omni Router endpoint with upstream attribution headers."""

    name: str = LLMProviderName.OMNI_ROUTER.value
    ...
    def __init__(
        ...
        *,
        ...
        include_usage: bool = True,
        site_url: str = "https://xplain.ai",
        app_name: str = "XplainAI",
        extra_headers: dict[str, str] | None = None,
    ) -> None:
        routing_headers = {
        ...
        super().__init__(
            ...
            name=LLMProviderName.OMNI_ROUTER.value,
        )
...
def build_llm_provider(settings: Settings) -> LLMProvider:
    ...
    Echo. ``LLM_PROVIDER=omni_router`` requires ``OMNI_ROUTER_API_KEY``.
    Use ``LLM_PROVIDER=echo`` explicitly for offline stubs.
    """
    if settings.llm_provider in (LLMProviderName.OMNI_ROUTER, "omni_router", "openrouter"):
        if settings.omni_router_api_key is None:
            raise RuntimeError(
                "LLM_PROVIDER=omni_router requires OMNI_ROUTER_API_KEY to be set (no silent Echo fallback)"
            )
        return OmniRouterProvider(
            base_url=settings.omni_router_base_url,
            api_key=settings.omni_router_api_key.get_secret_value(),
            timeout_seconds=settings.llm_request_timeout_seconds,
            site_url=settings.omni_router_site_url,
            app_name=settings.omni_router_app_name,
        )
