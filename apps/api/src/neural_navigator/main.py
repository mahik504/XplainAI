...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    cost_circuit_breaker = CostCircuitBreaker(settings=settings)

    app.state.event_bus = event_bus
    app.state.llm_service = llm_service
    app.state.conversation_store = conversation_store
    app.state.cost_circuit_breaker = cost_circuit_breaker
    ...
    _logger.info(
        ...
        budget_breaker_enabled=cost_circuit_breaker.enabled,
    )
    ...
    try:
        ...
        app.state.cost_circuit_breaker = None
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app.add_middleware(
        ...
        allow_headers=["Authorization", "Content-Type", HEADER_REQUEST_ID, "X-API-Key", "X-Tier"],
