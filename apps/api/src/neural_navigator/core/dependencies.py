...
def get_cost_circuit_breaker(request: Request) -> Any:
    from neural_navigator.services.circuit_breaker import CostCircuitBreaker

    breaker: CostCircuitBreaker | None = getattr(request.app.state, "cost_circuit_breaker", None)
    if breaker is None:
        settings = get_app_settings(request)
        breaker = CostCircuitBreaker(settings=settings)
        request.app.state.cost_circuit_breaker = breaker
    return breaker
...
CostCircuitBreakerDep = Annotated[Any, Depends(get_cost_circuit_breaker)]
