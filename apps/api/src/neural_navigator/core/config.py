...
class Settings(BaseSettings):
    ...
    # --- Embeddings & RAG --------------------------------------------------
    embedding_provider: str = "mock"  # "openai", "fastembed", "mock"
    ...
    # --- Model providers ---------------------------------------------------
    # Production / demo path: openai or omni_router. Echo remains the offline fallback.
    llm_provider: LLMProviderName = LLMProviderName.OPENAI
    llm_base_url: str = "https://api.openai.com/v1"
    openai_api_key: SecretStr | None = None
    anthropic_api_key: SecretStr | None = None
    google_api_key: SecretStr | None = None
    omni_router_api_key: SecretStr | None = None
    omni_router_base_url: str = "https://openrouter.ai/api/v1"
    omni_router_site_url: str = "https://xplain.ai"
    omni_router_app_name: str = "XplainAI"
    omni_router_daily_budget_usd: float = Field(default=10.0, gt=0.0)
    omni_router_hourly_budget_usd: float = Field(default=2.0, gt=0.0)
    omni_router_ip_budget_usd: float = Field(default=0.50, gt=0.0)
    ...
    # --- Public Demo Mode & Cost Circuit Breakers --------------------------
    demo_mode_enabled: bool = True
    budget_daily_limit_usd: float = Field(default=10.0, gt=0.0)
    budget_hourly_limit_usd: float = Field(default=2.0, gt=0.0)
    budget_ip_daily_limit_usd: float = Field(default=0.50, gt=0.0)
    budget_circuit_breaker_enabled: bool = True
    ...
    @field_validator("llm_base_url", "omni_router_base_url")
    @classmethod
    ...
    def _enforce_deployment_invariants(self) -> Settings:
        ...
        if self.llm_provider is LLMProviderName.OMNI_ROUTER and self.omni_router_api_key is None:
            problems.append("LLM_PROVIDER=omni_router requires OMNI_ROUTER_API_KEY")
        if problems:
