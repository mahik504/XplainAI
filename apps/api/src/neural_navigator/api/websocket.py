...
def get_ws_llm_service(websocket: WebSocket) -> LLMService:
    value = getattr(websocket.app.state, "llm_service", None)
    if not isinstance(value, LLMService):
        raise WebSocketException(
            code=WSCloseCode.TRY_AGAIN_LATER,
            reason="llm_service is not initialised",
        )
    return value


def get_ws_event_bus(websocket: WebSocket) -> EventBus:
    value = getattr(websocket.app.state, "event_bus", None)
    if not isinstance(value, EventBus):
        raise WebSocketException(
            code=WSCloseCode.TRY_AGAIN_LATER,
            reason="event_bus is not initialised",
        )
    return value


WSLLMServiceDep = Annotated[LLMService, Depends(get_ws_llm_service)]
WSEventBusDep = Annotated[EventBus, Depends(get_ws_event_bus)]
WSSettingsDep = Annotated[Settings, Depends(get_ws_app_settings)]
...
class ChatSocketSession:
    ...
    async def _execute_run(self, run_id: str, frame: ChatSendFrame) -> None:
        ...
        if (
            frame.custom_api_base
            and frame.custom_api_key
            and frame.custom_api_base.strip()
            and frame.custom_api_key.strip()
        ):
            from neural_navigator.services.llm import LLMService, OpenAICompatibleProvider

            provider = OpenAICompatibleProvider(
                base_url=frame.custom_api_base.strip(),
                api_key=frame.custom_api_key.strip(),
        ...
        # Circuit breaker pre-flight check for demo / public runs
        client_ip = (
            self._websocket.headers.get("x-forwarded-for", "").split(",")[0].strip()
            or (self._websocket.client.host if self._websocket.client else "127.0.0.1")
        )
        circuit_breaker = getattr(self._websocket.app.state, "cost_circuit_breaker", None)
        if circuit_breaker is None:
            from neural_navigator.services.circuit_breaker import CostCircuitBreaker

            circuit_breaker = CostCircuitBreaker(settings=self._settings)
            self._websocket.app.state.cost_circuit_breaker = circuit_breaker

        if circuit_breaker and getattr(self._settings, "budget_circuit_breaker_enabled", True):
            est_cost = circuit_breaker.estimate_cost(model)
            allowed, reason = await circuit_breaker.async_check_preflight(client_ip, est_cost)
            if not allowed:
                self._log.warning(
                    "ws.circuit_breaker.tripped",
                    run_id=run_id,
                    ip=client_ip,
                    reason=reason,
                )
                await self._events.emit(
                    ...
                    payload={"run_id": run_id, "error_code": ErrorCode.BUDGET_EXCEEDED.value},
                )
                await self._send(
                    ErrorFrame(
                        code=ErrorCode.BUDGET_EXCEEDED,
                        message=reason or "Budget limit reached for demo mode",
                        run_id=run_id,
                    )
                )
                await self._send(
        ...
        try:
            ...
            if (
                circuit_breaker
                and usage
                and getattr(self._settings, "budget_circuit_breaker_enabled", True)
            ):
...
async def chat_socket(
    ...
    llm: WSLLMServiceDep,
    events: WSEventBusDep,
    settings: WSSettingsDep,
) -> None:
