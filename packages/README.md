...
Nothing here imports from `apps/web/` or `apps/api/`; the dependency arrow only ever
points inward.

```
packages/
├── contracts/       Hand-authored specifications (the source of truth)
├── contracts-ts/    TypeScript types and Zod validators (@neural-navigator/contracts)
├── contracts-py/    Python Pydantic v2 models (nn-contracts)
└── codegen/         Generator and validation configuration
```
