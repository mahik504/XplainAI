...
redis-cli -u "$REDIS_URL" --scan --pattern "xplainai:budget:*" | xargs -r redis-cli -u "$REDIS_URL" del
redis-cli -u "$REDIS_URL" --scan --pattern "xplainai:ratelimit:*" | xargs -r redis-cli -u "$REDIS_URL" del
...
# Expected: HTTP 200 {"status":"ok","service":"xplainai-api","version":"3.0.0","uptime_seconds":...}
```
