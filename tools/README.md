...
| Directory | Contents |
| --- | --- |
| `codegen/` | Language-specific generator entry points invoked by `make contracts-gen`, notably `generate_python_models.py`, which turns the OpenAPI and AsyncAPI documents into Pydantic models under `packages/contracts-py/src/nn_contracts/generated/`. The TypeScript half is driven by the npm package in `packages/codegen/`. |
